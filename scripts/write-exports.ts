// Writes the static export artifacts (issues #146, #147, #148) for every
// theme: base16/base24 scheme YAML, static per-theme CSS, and per-theme DTCG
// design tokens. Split out of
...
import { buildDtcgJson } from '../src/dtcg.js';
...
export function writeExportArtifacts(
  ...
  for (const theme of themes) {
    ...
    writeFileSync(join(tokensDir, `${theme.slug}.tokens.json`), buildDtcgJson(theme));
  }
}
