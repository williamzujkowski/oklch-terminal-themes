...
export function convertHexToColor(hex: string): ColorValue {
  ...
  if (ok === undefined) {
    throw new Error(`Conversion to OKLCH failed: ${hex}`);
  }

  const oklch: Oklch = {
    l: round(clamp(ok.l ?? 0, 0, 1), 4),
    c: round(clamp(ok.c ?? 0, 0, 0.5), 4),
    h: round(Number.isFinite(ok.h) ? (ok.h as number) : 0, 1),
  };
...
export function roundTripDeltaE(hex: string): number {
  ...
  if (ok === undefined) throw new Error(`OKLCH conv failed: ${hex}`);
  const back = toRgb(ok);
  if (back === undefined) throw new Error(`RGB conv failed: ${hex}`);
...
export function hexFromOklch(oklch: Oklch): string {
  ...
  if (rgb === undefined) throw new Error('Failed to convert OKLCH to RGB');
