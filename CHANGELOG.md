...
**0.7.0 could not be imported by any consumer.** `dist/schema.js` and
`dist/sources.js` both do `import { z } from 'zod'` at module top level, but
`zod` was declared in `devDependencies`. A production install got:

```
ERR_MODULE_NOT_FOUND: Cannot find package 'zod' imported from
  node_modules/@williamzujkowski/oklch-terminal-themes/dist/schema.js
```

`zod` moves to `dependencies`. Verified by packing the real tarball and
installing it into a scratch consumer with `--omit=dev`: all 27 exports
import cleanly, and `zod` is present in the consumer's `node_modules`.

**Why nothing caught it.** `pnpm` hoists devDependencies in this workspace, so
every local and CI check passed — the one configuration that matters, a real
consumer doing a production install, was the one configuration never tested.
The `Packed-tarball consumer test` job now does exactly that on every PR, so
this class of failure cannot ship again. It is the same gap #169 pointed at
from a different direction.

This release is otherwise the accumulated work since 0.7.0; the entries below
were pending under Unreleased and ship here.
...
`CVD_SAFE_THRESHOLD` was well argued; several thresholds beside it were bare numbers. Each now records where it came from and how the 644-theme corpus actually splits at that value, measured rather than asserted:
...
- **`chromaTag` at 0.15 / 0.08** — chosen by inspection, with no natural boundary to snap to: mean chroma runs 0.0000–0.1931 (median 0.0916) with no gap near either cut. 18 themes (2.8%) are `vibrant`, 200 (31.1%) `muted`. Themes 0.0014 apart land on opposite sides.
- **Legacy `high-contrast` / `low-contrast` at 10 / 5** — documented rather than changed, since changing them would silently reclassify themes for existing consumers. `high-contrast` matches 67.2% of the corpus, and `low-contrast` **overlaps `wcag-aa`**: six themes carry both tags at once.
...
- **`CHROMATIC_CURSOR_THRESHOLD = 0.05`** (`src/accent.ts`) — a genuine judgement call, with the distribution table showing no gap: 69 themes (10.7%) sit in `[0.03, 0.07)`.
